...
```{r Q1 +++: Which are the stops/routes that has been changed?}
## Low priority

```

```{r Q5: Predict seat availability}
## Variables to merge:
# VSYS (categorical)
# Tagtyp_ID (categorical) - no need to merge
# GPS- first merge punkte to halte_2 by halt_id, then to df by ""
# Normalize -mean/sd

options(scipen = 999) #Stop showing in scientific notation. To reverse, set param to 0.

# Existing datasets:
halte_2 <- read.csv("data/fahrgastzahlen_2023/haltestelle.csv", sep=",")
colnames(df)
colnames(linie)
colnames(tagtyp)
colnames(halte)
