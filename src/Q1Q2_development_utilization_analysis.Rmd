---
title: "NavigatingZH_Data_Science_Intro_UZH_Q1Q2"
author: "Weijia Zhong"
date: "2024-05-08"
...
```{r Explorations on the datasets, echo=T, results='hide'}
...
Q1.1:Spatial coverage of VBZ system across the years
Datasets of previous years were downloaded from <br>
https://data.stadt-zuerich.ch/dataset/vbz_fahrgastzahlen_ogd <br>
and <br>
https://data.stadt-zuerich.ch/dataset/vbz_fahrzeiten_ogd

The goal is to compute the distance/passenger volumn within the whole VBZ system and compare across the years. <br>
Futher, contributions from different vehicle types (tram, bus, trolleybus, seilbahn, forchbahn, nachtnetz) are visualized in stacked barplot.
...
```{r Q1+: Change in traffic subcategories (tram, bus, cable car,…)}
...
```
```{r Q1+: Compiling into stacked barplot_Distance}
...
```{r Q1+: Compiling into stacked barplot_Volume}
...
print(p)
```
## The purpose of this question is to compare with the data collected with other major cities of the world. 
However, it's difficult to find fair comparisons because publicly available data are either Metro only or not at the level of a single city. Here I tried to make a separate set of tram only to see how it works.
...
```
```{r Q1++: Heatmaps of traffic volumn by hours for each vehivle type}
